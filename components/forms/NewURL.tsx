...
const formSchema = z.object({
  application: z.string().min(2).max(100),
  type: z.string().min(2).max(100),
  link: z.string().min(2).max(250),
  environment: z.string().min(2).max(150)
    ...
    fetch("/api/users/profile")
      ...
      .then((val) => {
        ...
        if (val.data?.role === 'secondaryadmin') {
          fetch("/api/applications/fetchAdminApps?userId=" + val.data?._id, { cache: 'no-store' })
            .then((data) => data.json())
